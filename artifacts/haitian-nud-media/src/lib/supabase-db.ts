...
export async function requestDownload(videoId: string, userId: string, isVip: boolean, freeDownloadsUsed: number): Promise<{ url: string; remaining: number }> {
...
export async function createComment(videoId: string, userId: string, body: string, anonymous: boolean, displayName: string) {
...
export async function getAdminStats(): Promise<AdminStats> {
  ...
  const { data: vips } = await supabase.from('users').select('*', { count: 'exact', head: true }).eq('plan', 'vip').gt('subscription_ends_at', new Date().toISOString());
  const { data: vids } = await supabase.from('videos').select('*', { count: 'exact', head: true });
  const { data: views } = await supabase.from('videos').select('views');
  const { data: downs } = await supabase.from('downloads').select('*', { count: 'exact', head: true });
  const { data: tickets } = await supabase.from('tickets').select('*', { count: 'exact', head: true }).eq('status', 'open');

  const totalViews = (views || []).reduce((sum: number, v: any) => sum + (v.views || 0), 0);

  return {
    totalUsers: users?.length ?? 0,
    activeVip: vips?.length ?? 0,
    totalVideos: vids?.length ?? 0,
    totalViews,
    totalDownloads: downs?.length ?? 0,
...
export async function adminListTickets(): Promise<SupportTicket[]> {
...
export async function adminReplyTicket(id: string, reply: string) {
  const { error } = await supabase.from('tickets').update({ reply, status: 'answered' }).eq('id', id);
  if (error) throw error;
}
